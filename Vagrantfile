...
Vagrant.configure(VAGRANTFILE_API_VERSION) do |config|
  ...
  # determine which demo apps to install, based on APPS environment variable (default=ALL)
  apps = ENV['APPS'] || "ALL"
  ...
  config.vm.provision "shell", path: "./install_scripts/postgres.sh"
  config.vm.provision "shell", path: "./install_scripts/fits.sh", args: shared_dir, privileged: false
  config.vm.provision "shell", path: "./install_scripts/demo-app.sh", args: apps, privileged: false
